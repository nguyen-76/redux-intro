...
const RootComponent = (props) => {
  ...
  const removeProductFromCart = (removedProduct) => {
    let removedTotalPrice = cart.totalPrice;
    const removedProductList = cart.products.map((cartProduct) => {
      if (removedProduct.id === cartProduct.id && cartProduct.qty > 0) {
        cartProduct.qty -= 1;
        cartProduct.price -= removedProduct.price;
        removedTotalPrice -= removedProduct.price;
      }
      return cartProduct;
    })
    setCart({products: removedProductList, totalPrice: removedTotalPrice})
  }
  // Step 3
    ...
    <WrapperBox>
      ...
      <Grid container spacing={2} p="1rem">
        <Grid item md={6}>
          ...
          removeProducts={removeProductFromCart} />
        </Grid>
        <Grid item md={6}>
          <CartPage cart={cart} />
        </Grid>
    ...
    <WrapperBox>
      ...
      <Grid container spacing={2} p="1rem">
        <Grid item sm={6}>
          ...
          removeProducts={props.removeProducts} />
        </Grid>
        <Grid item sm={6}>
          <ProductTwo 
  ...
  return (
    <WrapperBox>
      ...
      <Grid container spacing={2} p="1rem">
        <Grid item md={6}>
          <CartProductOne 
          product={props.cart.products[0]} />
        </Grid>
        <Grid item md={6}>
          <CartProductTwo 
          product={props.cart.products[1]} />
        </Grid>
        <Grid item md={12}>
      ...
      <Grid container justifyContent="center">
        ...
        <Grid item xs={8} >
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <Button variant="success" sx={{ width: "5rem" }} onClick={() => props.addProducts(props.product)}>
              Add
            </Button>
            <Button variant="error" sx={{ width: "5rem" }} onClick={() => props.removeProducts(props.product)}>
              Remove
            </Button>
          </div>
        </Grid>
  ...
  return (
    <WrapperBox>
      ...
      <Grid container justifyContent="center">
        ...
        <Grid item xs={8} >
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <Button variant="success" size="sm" style={{ width: "5rem" }} onClick={() => props.addProducts(props.product)}>
              Add
            </Button>
            <Button variant="error" size="sm" style={{ width: "5rem" }} onClick={() => props.removeProducts(props.product)}>
              Remove
            </Button>
          </div>
        </Grid>
